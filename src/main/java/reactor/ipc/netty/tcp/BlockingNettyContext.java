...
public class BlockingNettyContext {
	...
	public BlockingNettyContext(Maybe<? extends NettyContext> contextAsync,
			String description) {
	...
	public BlockingNettyContext(Maybe<? extends NettyContext> contextAsync,
		...
		this.context = contextAsync
				.timeout(lifecycleTimeout, unit, Maybe.error(new TimeoutException(description + " couldn't be started within " + unit.toMillis(lifecycleTimeout) + "ms")))
				.blockingGet();
	}
	...
	public void shutdown() {
		...
		context.dispose();
		context.onClose()
		       .timeout(lifecycleTimeout, unit, Completable.error(new TimeoutException(description + " couldn't be stopped within " + unit.toMillis(lifecycleTimeout) + "ms")))
		       .blockingAwait();
	}
	...
	protected void shutdownFromJVM() {
		...
		context.dispose();
		context.onClose()
		       .timeout(lifecycleTimeout, unit, Completable.error(new TimeoutException(description +
