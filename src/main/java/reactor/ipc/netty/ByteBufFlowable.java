...
public final class ByteBufFlowable extends Flowable<ByteBuf> {
	...
	public Flowable<InputStream> asInputStream() {
		return map(ByteBufPerhaps.ReleasingInputStream::new);
	}
	 ...
	 *
	 * @return {@link ByteBufPerhaps} of aggregated {@link ByteBuf}
	 */
	public ByteBufPerhaps aggregate() {
		return Perhaps.using(alloc::compositeBuffer,
				b -> Perhaps.fromSingle(this.reduce(b, (prev, next) -> prev.addComponent(next.retain())))
				         .doOnNext(cbb -> cbb.writerIndex(cbb.capacity()))
				         .filter(ByteBuf::isReadable),
				ByteBuf::release).to(ByteBufPerhaps::new);
	}
	 ...
	 */
	public ByteBufPerhaps multicast() {
