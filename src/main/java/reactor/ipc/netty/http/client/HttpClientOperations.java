...
class HttpClientOperations extends HttpOperations<HttpClientResponse, HttpClientRequest>
	...
	static HttpOperations bindHttp(Channel channel,
			BiFunction<? super HttpClientResponse, ? super HttpClientRequest, ? extends CompletableSource> handler,
	...
	public Completable send() {
		if (markSentHeaderAndBody()) {
			...
			return FutureCompletable.deferFuture(() -> channel().writeAndFlush(request));
		}
		else {
	...
	public WebsocketOutbound sendWebsocket(String subprotocols) {
		Completable m = withWebsocketSupport(websocketUri(), subprotocols, noopHandler());
	...
	@Override
	public Completable receiveWebsocket(String protocols,
	...
	final Completable withWebsocketSupport(URI url,
			String protocols,
			BiFunction<? super WebsocketInbound, ? super WebsocketOutbound, ? extends CompletableSource> websocketHandler) {
		...
		if (markSentHeaders()) {
			...
			if (replace(ops)) {
				Completable handshake = FutureCompletable.from(ops.handshakerResult)
				                                 .andThen(Completable.defer(() -> websocketHandler.apply(
						                                 ops,
						                                 ops)));
				if (websocketHandler != noopHandler()) {
					handshake = handshake.doOnEvent(ops);
				}
				return handshake;
			}
		}
			...
			if(ops != null) {
				...
				if (websocketHandler != noopHandler()) {
					handshake =
							handshake.andThen(Completable.defer(() -> websocketHandler.apply(ops, ops))
							         .doOnEvent(ops));
				}
				return handshake;
			}
	...
	static final class FlowableSendForm extends Flowable<Long> {
		...
		void _subscribe(Subscriber<? super Long> s) {
			if (!parent.markSentHeaders()) {
				EmptySubscription.error(new IllegalStateException("headers have already " + "been sent"), s);
				return;
			}
